    ...
    class Concat
      ...
      def initialize(inputs, output_without_extension)
        ...
        unless inputs.is_a?(Hash)                                                     and 
          ...
          raise MediaEditing::Video::Error.new("inputs must be an Hash with #{FORMATS.inspect} as keys and an array of strings as values with at least one value", inputs: inputs, output_without_extension: output_without_extension)
        end
        ...
        if mp4_inputs.size != webm_inputs.size
          raise MediaEditing::Video::Error.new('inputs[:mp4] and inputs[:webm] must be of the same size', inputs: @inputs, output_without_extension: @output_without_extension)
        end
