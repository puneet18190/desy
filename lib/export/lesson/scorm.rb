require 'fileutils'
require 'pathname'
require 'zlib'
require 'zip'
require 'env_relative_path'
require 'export'
require 'export/lesson'
require 'export/lesson/shared'

module Export
  
  module Lesson
    
    class Scorm
      include EnvRelativePath
      include Shared
      
      FOLDER                     = env_relative_pathname Rails.public_pathname, 'lessons', 'exports', 'scorms'
      ASSETS_FOLDER              = Lesson::FOLDER.join 'scorms', 'assets'
      ASSETS_SCORM_FOLDER_NAME   = 'assets'
      COMPRESSION_METHOD = Zip::Entry::STORED
      INDEX_PAGE_NAME = 'index.html'
      
      def self.remove_folder!
        FileUtils.rm_rf FOLDER
      end
      
      attr_reader :lesson, :rendered_slides, :filename_without_extension, :folder, :filename, :scorm_root_folder, :scorm_html_folder, :path
      
      def initialize(lesson, rendered_slides)
        @lesson, @rendered_slides = lesson, rendered_slides
        parameterized_title = lesson.title.parameterize
        time                = lesson.updated_at.utc.strftime(WRITE_TIME_FORMAT)
        @filename_without_extension = lesson.id.to_s.tap{ |s| s << "_#{parameterized_title}" if parameterized_title.present? }
        @folder                     = FOLDER.join lesson.id.to_s, time
        @filename                   = "#{filename_without_extension}.zip"
        @path                       = folder.join filename
        @scorm_root_folder          = Pathname filename_without_extension
        @scorm_html_folder          = "#{@scorm_root_folder}/html/"
        @assets_scorm_folder        = scorm_html_folder.join ASSETS_SCORM_FOLDER_NAME
        @math_images_folder         = scorm_html_folder.join MATH_IMAGES_ARCHIVE_FOLDER_NAME
      end
      
      def url
        find_or_create
        "/#{path.relative_path_from Rails.public_pathname}"
      end
      
      def find_or_create
        return if path.exist?
        raise "Assets are not compiled. Please create them using rake exports:lessons:scorms:assets:compile" unless assets_compiled?
        remove_old_files if folder.exist?
        folder.mkpath
        create
      end
      
      private
      
      def assets_compiled?
        ASSETS_FOLDER.exist? && ASSETS_FOLDER.entries.present?
      end
      
      def assets_files
        Pathname.glob ASSETS_FOLDER.join('**', '*')
      end
      
      def create
        Zip::File.open(path, Zip::File::CREATE) do |scorm|
          rendered_slides.each do |slide_id, rendered_slide|
            add_string_entry scorm, rendered_slide, scorm_html_folder.join("slide#{slide_id}.html")
          end
          assets_files.each do |path|
            add_path_entry scorm, path, assets_scorm_folder.join(path.relative_path_from ASSETS_FOLDER)
          end
          media_elements_files.each do |path|
            add_path_entry scorm, path, scorm_html_folder.join(path.relative_path_from MEDIA_ELEMENTS_UPFOLDER)
          end
          documents_files.each do |path|
            add_path_entry scorm, path, scorm_html_folder.join(path.relative_path_from DOCUMENTS_UPFOLDER)
          end
          math_images.each do |path|
            add_path_entry scorm, path, math_images_folder.join(path.basename)
          end
        end
        path.chmod 0644
      rescue
        path.unlink if path.exist?
        raise
      end
      
    end
    
  end
  
end
