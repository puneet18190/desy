  ...
  module Video
    module Editing
      describe Conversion, slow: true do
        ...
        def temp_folder
          @temp_folder ||= "#{Rails.root}/tmp/media/video/editing/conversions/#{Rails.env}/#{model.id}"
        end
            ...
            supported_formats.each do |format|
              context "with #{format} format", format: format do
                ...
                it 'creates the video cover' do
                  expect(File.exist?(model.media.path(:cover))).to be true
                end
  
                it 'creates the video thumb' do
                  expect(File.exist?(model.media.path(:thumb))).to be true
                end
