        ...
        describe Concat do
          ...
          context 'with a Sox unsupported format' do
            ...
            context 'with 1 audio' do
              ...
              its(:to_s) { should == "( ( avconv -i concat\\ 0.m4a -f sox - | pad 1.23 5.68 ) ) | sox -p -p | avconv -f sox -i - -strict experimental -c:a aac out\\ put.m4a" }
              context 'with empty paddings' do
                let(:audios_with_paddings) { [ ['concat 0.m4a', [0, 0.0] ] ] }
                its(:to_s) { should == "( ( avconv -i concat\\ 0.m4a -f sox - ) ) | sox -p -p | avconv -f sox -i - -strict experimental -c:a aac out\\ put.m4a" }
              end
            end
          
            context 'with 2 audios' do
              let(:audios_with_paddings) { [ ['concat 0.m4a', [1.234, 5.678] ], ['concat 1.m4a', [8.765, 4.321] ] ] }
              its(:to_s) { should == "( ( avconv -i concat\\ 0.m4a -f sox - | pad 1.23 5.68 ) ; ( avconv -i concat\\ 1.m4a -f sox - | pad 8.77 4.32 ) ) | sox -p -p | avconv -f sox -i - -strict experimental -c:a aac out\\ put.m4a" }
            end
          
            context 'with 3 audios' do
              let(:audios_with_paddings) { [ ['concat 0.m4a', [1.234, 5.678] ], ['concat 1.m4a', [8.765, 4.321] ], ['concat 3.m4a', [12.34, 56.78] ] ] }
              its(:to_s) { should == "( ( avconv -i concat\\ 0.m4a -f sox - | pad 1.23 5.68 ) ; ( avconv -i concat\\ 1.m4a -f sox - | pad 8.77 4.32 ) ; ( avconv -i concat\\ 3.m4a -f sox - | pad 12.34 56.78 ) ) | sox -p -p | avconv -f sox -i - -strict experimental -c:a aac out\\ put.m4a" }
            end
          end
