  ...
  module Audio
    module Editing
      describe Conversion, slow: true do
        ...
        def temp_folder
          @temp_folder ||= "#{Rails.root}/tmp/media/audio/editing/conversions/#{Rails.env}/#{model.id}"
        end
            ...
            context "with #{format} format", format: format do
              ...
              context "with a valid_audio" do
                ...
                before(:all) do
                  ...
                  [ stdout_log, stderr_log, temp, output_folder ].each { |f| FileUtils.rm(f) if File.exist?(f) }
                  subject
                end
                ...
                it "creates the stdout log" do
                  expect(File.exist?(stdout_log)).to be true
                end

                it "creates the stderr log" do
                  expect(File.exist?(stderr_log)).to be true
                end

                it "does not delete the temporary folder" do
                  expect(File.exist?(temp_folder)).to be true
                end

                it "does not delete the temporary audio" do
                  expect(File.exist?(temp)).to be true
                end
              end
              ...
              context 'with an invalid audio' do
                ...
                before do
                  ...
                  FileUtils.rm(temp) if File.exist?(temp)
                end
              ...
              context 'when uploaded_path file and temporary file do not exist' do
                ...
                before do
                  ...
                  FileUtils.rm uploaded_path if File.exist? uploaded_path
                  subject
                  FileUtils.rm temp if File.exist? temp
                end
        ...
        describe '#run' do
          ...
          context 'with a valid audio' do
            ...
            it 'deletes the temporary folder' do
              expect(File.exist?(temp_folder)).to be false
            end  
