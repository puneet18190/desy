...
class UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save
      UserMailer.account_confirmation(@user, request.protocol, request.host, request.port).deliver
  ...
  def send_reset_password
    ...
    if user = User.active.confirmed.where(email: email).first
      user.password_token!
      UserMailer.new_password(user, request.protocol, request.host, request.port).deliver
    end
  ...
  def reset_password
    ...
    if new_password
      UserMailer.new_password_confirmed(user, new_password, request.protocol, request.host, request.port).deliver
