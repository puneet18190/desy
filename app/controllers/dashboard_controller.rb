...
class DashboardController < ApplicationController
  ...
  #
  def index
    @lessons = current_user.suggested_lessons(@lessons_for_page * @lesson_raws)
    @lessons_emptied = Lesson.dashboard_emptied? current_user.id
    @media_elements = current_user.suggested_media_elements(@media_elements_for_page * @media_element_raws)
